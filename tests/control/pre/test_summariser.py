...
from app.control.pre.summariser import pre_process, _split_by_token_length
...
@pytest.mark.parametrize("max_input_tokens, expected_number_of_splits", TOKEN_SPLIT_VALID_DATA)
def test_pre_process(max_input_tokens, expected_number_of_splits, mock_tokenizer, valid_conversation_dict):
    with patch('app.control.pre.summariser.AutoTokenizer.from_pretrained', return_value=mock_tokenizer):
        result = pre_process(conversation_dict=valid_conversation_dict, max_input_tokens=max_input_tokens)
...
@pytest.mark.parametrize("max_input_tokens, expected_number_of_splits", TOKEN_SPLIT_VALID_DATA)
def test_split_by_token_length(max_input_tokens, expected_number_of_splits, mock_tokenizer, valid_conversation_dict):
    with patch('app.control.pre.summariser.AutoTokenizer.from_pretrained', return_value=mock_tokenizer):
